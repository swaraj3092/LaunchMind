          ...
          <AnimatePresence mode="wait">
            ...
            {tab === "pitchdeck" && (
              <motion.div key="pitchdeck"
                ...
                <div className="space-y-4">
                  {[
                    { num: "01", title: "Cover Slide", icon: "🚀", color: V, content: [`Company: Your Startup Name`, `Tagline: ${analysis?.idea_summary?.split('.')[0] || idea}`, `Presenter: Your Name | ${role}`, `Date: ${new Date().toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })}`] },
                    ...
                    { num: "07", title: "Competitive Advantage", icon: "🏆", color: M, content: [
                      ...(plan?.competitors?.slice(0, 2).map((c: any) => `vs ${c.name}: ${c.differentiator?.slice(0, 60)}...`) || []),
                    ...
                    { num: "09", title: "Traction & Roadmap", icon: "📈", color: V, content: [
                      ...(plan?.roadmap?.day_30?.slice(0, 2).map((m: string) => `30 Days: ${typeof m === 'string' ? m.replace(/\[.*?\]\s*/, '') : m}`) || []),
                      ...(plan?.roadmap?.day_60?.slice(0, 1).map((m: string) => `60 Days: ${typeof m === 'string' ? m.replace(/\[.*?\]\s*/, '') : m}`) || []),
